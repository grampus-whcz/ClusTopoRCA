    ...
    for dataset_main, (dataset_subs, filename_prefix_pattern) in datasets.items():
        for dataset_sub in dataset_subs:
            ...
            for model in llm_models:
                ...
                # 构建文件路径
                input_filename = f"{filename_prefix}_no_RAG_c4_knowledge_graph_advanced_merged_{model}_extracted_tasks_info.md"
                input_path = os.path.join(base_path, dataset_main, model, input_filename)
                
                task_csv_filename = f"{filename_prefix}_no_RAG_c4_knowledge_graph_advanced_merged_{model}_task_summary.csv"
                global_csv_filename = f"{filename_prefix}_no_RAG_c4_knowledge_graph_advanced_merged_{model}_global_summary.csv"
                difficulty_csv_filename = f"{filename_prefix}_no_RAG_c4_knowledge_graph_advanced_merged_{model}_difficulty_summary.csv"
